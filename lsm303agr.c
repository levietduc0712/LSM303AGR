/**
  ...
  *          MEMS accelerometer.
  * @note    Viet Duc has contributed to the optimization of lsm303agr.c
  ******************************************************************************
  * @attention
  *
...
unsigned short LPen_bit = 0;
unsigned short HR_bit = 0;
...
{  
  uint8_t ctrl = 0x00;
  uint16_t ctrl_reg = 0x00;

  ctrl_reg = (uint16_t) InitStruct;

  LPen_bit = (ctrl_reg >> 3) & 0x1;
  HR_bit = (ctrl_reg >> 11) & 0x1;
...
void LSM303AGR_AccReadXYZ(float *pData)
{
  ...
  if (LPen_bit == 1 && HR_bit == 0) {
    /* Operating mode is Low-power mode 8-bit */
    shift = 8;
  } else if (LPen_bit == 0 && HR_bit == 0) {
    /* Operating mode is Normal mode 10-bit */
    shift = 6;
  } else if (LPen_bit == 0 && HR_bit == 1) {
    /* Operating mode is High-resolution mode 12-bit */
    shift = 4;
  } else {
    return 1;
  }
  ...
  if(!(ctrlx[0] & LSM303AGR_BLE_MSB)) 
  {
    for(i=0; i<3; i++)
    {
    	pnRawData[i]= (((int16_t)(buffer[2*i+1] << 8) + buffer[2*i]) >> shift);
    }
  }
  else /* Big Endian Mode */
  {
    for(i=0; i<3; i++)
    {
    	pnRawData[i]= (((int16_t)(buffer[2*i] << 8) + buffer[2*i+1]) >> 6);
    }
  }
  ...
  case LSM303AGR_FULLSCALE_2G:
    ...
    sensitivity = LSM303AGR_ACC_SENSITIVITY_2G_LOW_POWER_MODE;
  #endif
  #ifdef NORMAL_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_2G_NORMAL_MODE;
  #endif
  #ifdef HIGH_RESOLUTION_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_2G_HIGH_RESOLUTION_MODE;
  #endif
    break;
  case LSM303AGR_FULLSCALE_4G:
  #ifdef LOW_POWER_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_4G_LOW_POWER_MODE;
  #endif
  #ifdef NORMAL_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_4G_NORMAL_MODE;
  #endif
  #ifdef HIGH_RESOLUTION_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_4G_HIGH_RESOLUTION_MODE;
  #endif
    break;
  case LSM303AGR_FULLSCALE_8G:
  #ifdef LOW_POWER_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_8G_LOW_POWER_MODE;
  #endif
  #ifdef NORMAL_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_8G_NORMAL_MODE;
  #endif
  #ifdef HIGH_RESOLUTION_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_8G_HIGH_RESOLUTION_MODE;
  #endif
    break;
  case LSM303AGR_FULLSCALE_16G:
  #ifdef LOW_POWER_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_16G_LOW_POWER_MODE;
  #endif
  #ifdef NORMAL_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_16G_NORMAL_MODE;
  #endif
  #ifdef HIGH_RESOLUTION_MODE
    sensitivity = LSM303AGR_ACC_SENSITIVITY_16G_HIGH_RESOLUTION_MODE;
  #endif
    break;
  }
  ...
  for(i=0; i<3; i++)
  {
    // Convert (mg/LSB) to (g/LSB)
    pData[i]= (float)(pnRawData[i] * sensitivity * 0.001 * GRAVITY_EARTH); //
  }
}
